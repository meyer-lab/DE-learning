...
def importLINCS(cellLine):
    ...
    annotation = pd.read_csv(
        join(path_here, "de/data/", cellLine + "_genes.txt"), header=None, sep=" ")[0]
...
def importRNAseqKO():
    ...
    data = pd.read_csv(join(path_here, "de/data/rpmCounts_allRuns_matrix.tsv.xz"),
                       index_col="GeneSymbol", delim_whitespace=True)
...
def ImportMelanoma():
    ...
    data_combined = data_in.groupby(
        by=data_in.columns, axis=1).mean()  # knockout replicates
    data_combined = data_combined.groupby(
        ["GeneSymbol"]).max()  # duplicate genes
...
def importGenomeData():
    ...
    data1 = pd.read_csv(
        join(path_here, "de/data/GSE92742_Broad_LINCS_Level2.csv.xz"), compression="xz")
    data2 = pd.read_csv(
        join(path_here, "de/data/GSE70138_Broad_LINCS_Level2.csv.xz"), compression="xz")
    data = data1.join(data2)
    # Import perturbation IDs and names
    inst_info = pd.read_csv(join(
        path_here, "de/data/GSE106127_inst_info.txt"), sep="\t").set_index("inst_id")
    inst_info = inst_info.drop(
        ["pert_time", "pert_time_unit", "seed_seq_6mer", "seed_seq_7mer", "pert_itime"], axis=1)
    # Import gene IDs and names
    gene_info = pd.read_csv(join(
        path_here, "de/data/GSE92742_Broad_LINCS_gene_info.txt"), sep='\t', index_col="pr_gene_id")
    gene_info = gene_info.drop(
        ["pr_gene_title", "pr_is_lm", "pr_is_bing"], axis=1)
...
def determineCellTypes(inst_info):
    ...
    cell_types_counts = cell_types_counts.drop(
        ["pert_iname", "pert_type", "pert_time", "pert_time_unit", "seed_seq_6mer", "seed_seq_7mer", "pert_itime"], axis=1)
...
def cell_type_perturbations(data, inst_info, gene_info, cell_id):
    ...
    inst_info_celltype = inst_info.loc[((inst_info["cell_id"] == cell_id) | (inst_info["cell_id"] == (
        cell_id + ".311"))) & ((inst_info["pert_type"] == "ctl_vector") | (inst_info["pert_type"] == "trt_sh"))]
    drop_cols = []
    rename_cols = []
    new_names = []
    ...
    out_celltype.rename(columns=dict(
        zip(rename_cols, new_names)), inplace=True)
    # Average replicates
    ...
    out_celltype = out_celltype[[
        x for x in out_celltype if x not in ["LUCIFERASE"]] + ["LUCIFERASE"]]
    return out_celltype
...
def importgenes():
    ...
    path_here = dirname(dirname(__file__))
    x_data = pd.read_csv(
        join(path_here, "de/data/sumarizedResults.txt"), header=0, sep="\t")
    xdata = pd.DataFrame(x_data, columns=["target", "meanlFC"])
    y_data = pd.read_csv(
        join(path_here, "de/data/colonyGrowthResults_allhits.txt"), header=0, sep="\t")
